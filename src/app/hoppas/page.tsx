...
export default function HoppasPage() {
  return (
    <CategoryPage
      heading={cat.heading}
      intro={cat.intro}
      body="호빠는 여성 손님이 주인공이 되는 고급 나이트라이프 공간이다. 입장하면 매니저가 시스템을 안내하고, 테이블에 호스트가 배정되어 대화와 음료 서비스를 제공한다. 호스트 지명은 선택사항이며, 마음에 들지 않으면 교체를 요청할 수 있다. 2차 강요는 없고 본인이 원하는 만큼만 즐기면 된다. 이 점이 처음 방문하는 분들이 가장 궁금해하는 부분인데, 강압적인 분위기는 전혀 없으니 안심해도 된다. 안전 면에서도 대부분의 업소에 CCTV와 보안 인력이 상주하며, 문제 상황 발생 시 매니저가 즉시 대응한다. 주요 상권은 강남·역삼이 가장 크고, 부산 서면·해운대, 장안동, 건대입구 순으로 선택지가 넓다. 지역마다 분위기가 다른데, 강남은 세련되고 모던한 인테리어가 많고, 부산은 활기차고 친근한 분위기가 특징이다. 친구와 함께 가는 경우가 많지만 혼자 방문해도 전혀 어색하지 않도록 호스트가 자연스럽게 대화를 이끌어준다. 금·토 23시 전후가 가장 붐비는 시간대이고, 평일 저녁은 한적하게 여유를 즐기기 좋다. 놀쿨에서 전국 15곳의 시스템과 분위기를 미리 파악하고, 편안한 첫 방문을 준비하자."
      guide={{
        title: '처음이세요? 걱정 마세요',
        items: [
